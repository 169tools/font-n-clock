...
std::array<ink_extents, 10> digit_extents(const text_measurer &measurer)
{
	...
	for (int i = 0; i <= 9; ++i) {
		const std::optional<ink_extents> extents = measurer.measure(std::string(1, static_cast<char>('0' + i)));
		if (!extents) {
			return {};
		}
		digits[i] = extents.value();
	}
	return digits;
}
...
row_extents date_reference_extents(const text_measurer &measurer, const date_format format)
{
	...
	for (int month = 1; month <= 12; ++month) {
		for (int day = 1; day <= 31; ++day) {
			const std::optional<ink_extents> extents = measurer.measure(format_date(format, month, day, 0));
			if (!extents) {
				return {};
			}
	...
	for (int weekday = 1; weekday < 7; ++weekday) {
		const std::optional<ink_extents> extents =
			measurer.measure(format_date(format, widest_month, widest_day, weekday));
		if (!extents) {
			return {};
		}
		max_extents.width = std::max(max_extents.width, extents->width);
	...
	const std::optional<ink_extents> extents = measurer.measure(height_glyphs);
	if (!extents) {
		return {};
	}
...
row_extents time_reference_extents(const text_measurer &measurer, const bool twelve_hour)
{
	...
	for (int hour = first_hour; hour <= last_hour; ++hour) {
		for (int minute = 0; minute < 60; ++minute) {
			...
			const std::optional<ink_extents> extents = measurer.measure(text);
			if (!extents) {
				return {};
			}
			max_extents.extend(extents.value());
		}
	}
	return max_extents;
}
...
row_extents meridiem_reference_extents(const text_measurer &measurer)
{
	...
	for (const char *meridiem : meridiem_names) {
		const std::optional<ink_extents> extents = measurer.measure(meridiem);
		if (!extents) {
			return {};
		}
		max_extents.extend(extents.value());
